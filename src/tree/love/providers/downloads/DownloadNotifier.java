...
import static tree.love.providers.downloads.DownloadManager.Request.VISIBILITY_VISIBLE;
import static tree.love.providers.downloads.DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED;
import static tree.love.providers.downloads.DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_ONLY_COMPLETION;
import static tree.love.providers.downloads.Downloads.Impl.STATUS_RUNNING;
...
import tree.love.providers.downloads.util.LongSparseLongArray;
...
public class DownloadNotifier {
    ...
    private void updateWithLocked(Collection<DownloadInfo> downloads) {
        ...
        final HashMap<String, List<DownloadInfo>> clustered = new HashMap<String, List<DownloadInfo>>();

        for (DownloadInfo info : downloads) {
            final String tag = buildNotificationTag(info);
            if (tag != null) {
                List<DownloadInfo> collection = clustered.get(clustered);
                if (collection == null) {
                    collection = new ArrayList<DownloadInfo>();
